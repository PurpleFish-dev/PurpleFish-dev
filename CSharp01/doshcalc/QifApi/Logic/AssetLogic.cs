...
namespace QifApi.Logic
{
    internal static class AssetLogic
    {
        ...
        internal static List<BasicTransaction> Import(string transactionItems, ref QifDom.yearFormat yearFormat, ref QifDom.dayMonthFormat dayMonthFormat)
        {
		...
		internal static void Export(StreamWriter writer, List<Account> list)
		{
            if ((list != null) && (list.Count > 0))
            {
				...
				{
					AccountListLogic.Export(writer, account.AccountListTransaction);

					foreach (BasicTransaction item in account.Transactions)
					{
						writer.WriteLine(NonInvestmentAccountFields.Date + item.Date.ToShortDateString());
